...
func Publish(v *db.Video) *interact.Video {
	if v == nil {
		return nil
	}

	return &interact.Video{Id: v.Id, FavoriteCount: int64(v.FavoriteCount),
		CommentCount: int64(v.CommentCount), PlayUrl: v.PlayUrl,
		CoverUrl: v.CoverUrl, Title: v.Title}
}

// Publishs pack list of Publish info
func Publishs(us []*db.Video) []*interact.Video {
	Publishs := make([]*interact.Video, 0)
	for _, v := range us {
